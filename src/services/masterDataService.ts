...
// ✅ Fetch data from API
const fetchDataFromAPI = async (): Promise<MasterApiResponse[]> => {
  try {
    ...
    const mappedData: MasterApiResponse[] = rawData.map((record: any) => {
      ...
      return {
        ...
        surface_hardness: getNestedValue(record, ["Hardness @ 0", "1 mm", " (From Graph)(ALL-MEAN)"]),
...
const mapAPIDataToCollections = (apiData: MasterApiResponse[]) => {
...
export const processFromAPI = async (): Promise<{
  furnaces: IFurnace[];
  ...
  try {
    ...
    const createdCustomerProducts = await bulkCreateUniqueCustomerProducts(cpDataArray);
    ...
    const createdChartDetails = await bulkCreateUniqueChartDetails(chartDetailDataArray);
